...
@Component({
  selector: 'app-pagar-r',
  templateUrl: './pagar-r.page.html',
  styleUrls: ['./pagar-r.page.scss'],
})
export class PagarRPage implements OnInit {
  ...
  constructor(public navCtrl: NavController, private formBuilder: FormBuilder, public loadingCtrl: LoadingController, public api: ApiService,
  	...
  	this.precio = (this.route.snapshot.params.quantity*16).toFixed(2);
  	this.local_id = this.route.snapshot.params.id;
  	...
  	this.loadingCtrl.create().then(l=>{
	      ...
	      this.loadingCtrl.create().then(l=>{
	        // l.present();
		      ...
		      this.loadingCtrl.create().then(l=>{
	            l.present();
	            this.api.payCloset({user_id:this.user.id, local_id:this.local_id,quantity:this.route.snapshot.params.quantity}).subscribe(data=>{

	              this.events.publish('reloadCloset',data)

	              this.alertCtrl.create({message:"Gracias por su compra! Se ha generado un código QR que deberá mostrar cuando entregue sus prendas."}).then(a=>{a.present(); setTimeout(()=>{a.dismiss()},3000)});
